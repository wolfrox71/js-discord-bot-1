module.exports = {
    name: "info",
    alias: ["description"],
    description: "Returns the description of any command",
    execute(msg, args) {
        if (args.length < 1) {
        ...
        for (i in commands) {
            if (commands[i].name === args[0] || commands[i].alias.includes(args[0])) {
                msg.reply(`the description for the command "${commands[i].name}" is "${commands[i].description}"`); found = true; break}
        }
